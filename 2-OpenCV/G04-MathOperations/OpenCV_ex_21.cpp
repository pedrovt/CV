...
int main( int argc, char** argv )
{
	...
	// Negative
	cv::threshold(originalImage, originalImage, 90, 255, CV_THRESH_BINARY_INV);

	cv::namedWindow( "Imagem Original after threshold", cv::WINDOW_AUTOSIZE );
	cv::imshow( "Imagem Original after threshold", originalImage );

	// Dilate
	int erosion_size = 9;
    cv::Mat result;
	cv::Mat kernel = cv::getStructuringElement(0, cv::Size( erosion_size, erosion_size ));
    
	cv::erode(originalImage, result, kernel, cv::Point(-1, -1), 2);
